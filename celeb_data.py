...
class Celeba(Dataset):
    ...
    att_dict = {'5_o_Clock_Shadow': 0, 'Arched_Eyebrows': 1, 'Attractive': 2,
                ...
                'Wearing_Necklace': 37, 'Wearing_Necktie': 38, 'Young': 39, 'Other_Hair':40}

    def __init__(self, data_dir, atts, img_resize, batch_size, prefetch_batch=1, drop_remainder=True,
                 num_threads=1, shuffle=True, buffer_size=4096, repeat=-1, sess=None, part='train', crop=True):
        super(Celeba, self).__init__()

        list_file = os.path.join(data_dir, 'list_attr_celeba.txt')
        if crop:
            img_dir_jpg = os.path.join(data_dir, '')
            img_dir_png = os.path.join(data_dir, '')
        else:
            img_dir_jpg = os.path.join(data_dir, '')
            img_dir_png = os.path.join(data_dir, '')
        ...
        print("FUCK")
        print(img_dir_jpg, img_dir_png)
        print(names)
#         if os.path.exists(img_dir_png):
#             img_paths = [os.path.join(img_dir_png, name.replace('jpg', 'png')) for name in names]
        if os.path.exists(img_dir_jpg):
            print(img_dir_jpg)
#             img_paths = os.path.join(img_dir_jpg, str(names))
